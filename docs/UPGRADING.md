...
0. **Check the PHP version** (`php -v`, and the version your web server runs;
   ...
   **Docker users, same release:** the compose file moved to
   `docker/compose.yaml`, and it now pulls published images and includes Caddy
   on ports 80 and 443. The app no longer publishes port 8080. Settings moved
   from `.env.docker` to a `.env` next to `compose.yaml`: start from
   `docker/.env.example` and carry over `DB_PASSWORD` and `ENCRYPTION_KEY`
   (the `MARIADB_*` lines are gone). Drop the old
   `APP_BASE_URL=http://localhost:8080/`, and set `TICKETHUB_DOMAIN` and
   `TICKETHUB_TLS` instead (see [https.md](https.md)). If your own reverse
   proxy already does HTTPS, use `TICKETHUB_TLS=upstream` and point it at
   port 80. Before 1.1 the image couldn't actually serve pages (see the
   CHANGELOG), so there's no data to migrate.
