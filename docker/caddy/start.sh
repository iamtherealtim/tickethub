#!/bin/sh
# Writes /etc/caddy/Caddyfile from .env.docker, then runs Caddy.
#
...
#   TICKETHUB_CERT_FILE / TICKETHUB_KEY_FILE   names inside docker/certs/   (files; default tls.crt / tls.key)
...
fail() {
    ...
    printf '[caddy-setup] Fix .env.docker, then: docker compose up -d\n' >&2
...
need() {
    for v in "$@"; do
        eval "val=\${$v:-}"
        [ -n "$val" ] || fail "TICKETHUB_TLS=dns with TICKETHUB_DNS_PROVIDER=${PROVIDER} needs $v in .env.docker."
    done
}
...
if [ -z "$DOMAIN" ]; then
    ...
    files)
        ...
        [ -r "$CERT" ] || fail "TICKETHUB_TLS=files but ${CERT#/certs/} is not in docker/certs/. Put the certificate (with any intermediates appended) there as PEM."
        [ -r "$KEY" ] || fail "TICKETHUB_TLS=files but ${KEY#/certs/} is not in docker/certs/. Put the unencrypted PEM private key there."
        ...
        log "HTTPS for ${DOMAIN}: certificate from docker/certs/${CERT#/certs/}"
        SITE_ADDR="$DOMAIN"
        TLS="tls ${CERT} ${KEY}"
        ;;
    internal)
...
{
    echo "# Generated by tickethub-caddy on $(date -u +%Y-%m-%dT%H:%M:%SZ) from .env.docker — edits are overwritten."
    echo "{"
    [ -n "$GLOBAL" ] && printf '\t%s\n' "$GLOBAL"
    echo "}"
    echo ""
